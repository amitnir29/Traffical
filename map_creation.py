...
def create_map():
    ...
    """
    # part 1
    from_roads, all_traffic_lights, junctions_data = get_junctions_data()
    # part 2
    notified_lanes_dict: Dict[int, Set[int]] = get_notified_lanes_dict(set(from_roads), all_traffic_lights)
    roads: Dict[int, IRoadSection] = get_roads(notified_lanes_dict)
    # part 3
    traffic_lights = get_traffic_lights(all_traffic_lights, roads)
    # part 4
...
def get_junctions_data():
    ...
    all_traffic_lights: List[List[RoadLane]] = list()
    ...
    for junction_data in get_db_junctions():
        ...
        # add the traffic lights. we know that now traffic light can appear in two different junctions,
        # so no need to merge lists, just add all traffic lights to a long list
        all_traffic_lights += junction_data.traffic_lights
    return from_roads, all_traffic_lights, junctions_data


def get_notified_lanes_dict(all_road_ids: Set[int], all_traffic_lights) -> Dict[int, Set[int]]:
    """
    ...
    for traffic_light in all_traffic_lights:
        for pair in traffic_light:
            notified_lanes_dict[pair[0]].add(pair[1])
...
def get_roads(notified_lanes_dict: Dict[int, Set[int]]) -> Dict[int, IRoadSection]:
    roads: Dict[int, IRoadSection] = dict()
    # create all roads
    for road_data in get_db_road_sections():
        road_data: RoadData
        # make sure there are no errors in the db
        if road_data.idnum not in notified_lanes_dict:
            raise Exception(f"road id with no movements: {road_data.idnum}")
...
def get_traffic_lights(all_traffic_lights: List[List[RoadLane]], roads: Dict[int, IRoadSection]) -> List[ITrafficLight]:
    traffic_lights: List[ITrafficLight] = list()
    for road_lane_list in all_traffic_lights:
        # create a traffic light for the list, that controls the lanes in the list
        lanes_list = [roads[road_lane.road_id].get_lane(road_lane.lane_num) for road_lane in road_lane_list]
        # lanes_list should contain only notified lanes
        traffic_lights.append(TrafficLight(lanes_list))
    return traffic_lights


def set_lane_movements(roads: Dict[int, IRoadSection], from_roads: Dict[int, List[Tuple[RoadLane, RoadLane]]]):
...
def get_all_junctions(junctions_data: List[JunctionData], roads: Dict[int, IRoadSection]) -> List[IJunction]:
